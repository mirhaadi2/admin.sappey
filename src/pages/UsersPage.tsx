...
import { Button, Table, Pagination, ConfirmDialog, UserForm, type UserFormValues } from '@/components';
      ...
      render: (value: string) => (
        <span
          className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-semibold ${
            value === 'active' ? 'bg-emerald-100 text-emerald-800' : 'bg-rose-100 text-rose-800'
          }`}
        >
    ...
    <div className="space-y-6 min-h-screen bg-slate-50">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm font-medium">Search, view, edit, and manage users with full control.</p>
        </div>
      ...
      <Table
        ...
        rowActions={(user: AdminUser) => (
          <div className="flex items-center justify-end gap-2">
            <Button variant="outline" size="sm" icon={<Eye size={14} />} onClick={() => openViewModal(user)} />
            <Button variant="outline" size="sm" icon={<Pencil size={14} />} onClick={() => openEditModal(user)} />
            {/* <Button variant="secondary" size="sm" onClick={() => toggleBan(user)}>
              {user.status === 'banned' ? 'Unban' : 'Ban'}
            </Button> */}
            <Button variant="danger" size="sm" icon={<Trash size={14} />} onClick={() => handleDelete(user)} />
          </div>
        )}
      />
        ...
        <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="w-full max-w-lg bg-white rounded-2xl shadow-2xl p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setShowUserModal(null)} className="text-slate-400 hover:text-slate-600">
                ×
              </button>
            </div>
